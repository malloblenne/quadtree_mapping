...
for ( int i=0; i<4; i++) // x ogni figlio
   {
    ...
    if (robotfall==true && ptfall==false)
    ...
    else if ( robotfall==false && ptfall==false) // può essere attraversato dalla retta o meno
      {
         ...
         if ( (children[i]->getIfLeaf()==true) && (touch==true) && (intersection[0] > ((float)menace[0][0])) && (intersection[0] < ((float)menace[1][0])))
           { // tra xA e xB
...
3 4
*/
// uso dim, corner[0] e corner[1]
// array[4][2] contiene coppie di corner[0] e [1]
int newdim=dim/2; //this

//corner[0] (x) di 1
array[0][0]=corner[0];
//corner[1] (y) di 1
array[0][1]=corner[1]+newdim;

//corner[0] (x) di 2
array[1][0]=corner[0]+newdim;
//corner[1] (y) di 2
array[1][1]=corner[1]+newdim;
