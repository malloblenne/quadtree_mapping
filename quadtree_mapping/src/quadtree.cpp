...
for (int j=0; j<4; j++)
   {
    //children[j]=new Node(true,0,0,newdim,array[j][0],array[j][1]);
...
Quadtree:: ~Quadtree()
{
delete root; // ma farà da solo la ricorsione dei distruttori?
}
...
// se è all'interno chiamo l'insert del nodo, altrimenti devo creare nuovi nodi...e richiamare questa!! funzione (potrebbe servire ancora di incrementare la dim)
bool limit=false;
...
switch (childpos)
{
case 0: temproot= new Node(false,0,0,fatherdim,n->getCornX(),(n->getCornY()-n->getDim())); break;
case 1: temproot= new Node(false,0,0,fatherdim,(n->getCornX()-n->getDim()),(n->getCornY()-n->getDim())); break;
case 2: temproot= new Node(false,0,0,fatherdim,n->getCornX() ,n->getCornY()); break;
case 3: temproot= new Node(false,0,0,fatherdim,(n->getCornX() -n->getDim()),n->getCornY()); break;
default : break;
};
